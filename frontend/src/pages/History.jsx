    ...
    return (
        ...
        >
            <div className="flex justify-between items-end mb-12">
                <div>
                    <h1 className="text-5xl font-black text-black mb-2 tracking-tighter uppercase">Audit History</h1>
            ...
            {loading ? (
                ...
                <div className="bg-gray-50 border-2 border-dashed border-gray-200 rounded-3xl py-20 px-10 text-center">
                    <p className="text-gray-400 font-bold text-xl mb-2">No Audits Found</p>
                    <p className="text-gray-400 text-sm">Upload a survey recording from the Home page to start auditing.</p>
                </div>
            ) : (
                    ...
                    {history.map((item) => (
                        <div 
                            ...
                            className={`bg-white border rounded-2xl p-6 flex justify-between items-center transition-all cursor-pointer group shadow-sm hover:shadow-md ${
                                selectedAudit?.id === item.id ? 'border-black ring-1 ring-black' : 'border-gray-100 hover:border-black'
                            }`}
                        ...
                        >
                            <div className="flex items-center gap-6">
                                <div className={`w-14 h-14 rounded-xl flex items-center justify-center transition-colors ${
                                    ...
                                    {item.audit_result.status === 'Match' ? <CheckCircle2 className="w-7 h-7" /> : <AlertCircle className="w-7 h-7" />}
                                </div>
                                <div>
                                    <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">{item.timestamp}</p>
                            ...
                            <div className="flex items-center gap-10">
                                <div className="text-right">
                                    <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-1">Audit Status</p>
                                    <p className={`text-lg font-black uppercase leading-none ${item.audit_result.status === 'Match' ? 'text-green-500' : 'text-red-500'}`}>
                    ...
                    >
                        <div className="max-w-[1000px] mx-auto">
                            <div className="flex justify-between items-center mb-10">
                                ...
                                <div className="text-right">
                                    <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Selected Audit</p>
                                    <p className="text-lg font-black text-black uppercase">{selectedAudit.name}</p>
                                </div>
                            </div>
